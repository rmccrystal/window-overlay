...
pub trait WindowBuilderExtWindows {
    /// Hijacks a window instead of creating it __NEW
    fn with_custom_hwnd(self, hwnd: HWND) -> WindowBuilder;

    /// Run code after window is created
    fn with_hwnd_callback(self, callback: unsafe fn(HWND)) -> WindowBuilder;
...
impl WindowBuilderExtWindows for WindowBuilder {
    /// Changed from source __NEW
    #[inline]
    fn with_custom_hwnd(mut self, hwnd: HWND) -> WindowBuilder {
        self.platform_specific.custom_hwnd = Some(hwnd);
        self
    }

    #[inline]
    fn with_hwnd_callback(mut self, callback: unsafe fn(HWND)) -> WindowBuilder {
        self.platform_specific.hwnd_callback = Some(callback);
        self
    }
